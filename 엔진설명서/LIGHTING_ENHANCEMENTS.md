...
### 8.4 ToonPBREditable Dark Band Alpha (검은 영역 완화)

툰 쉐이딩의 **가장 어두운 밴드(검은 영역)** 를 연하게 만드는 전용 파라미터입니다.
- MaterialComponent: `toonPbrShadowAlpha` (0~1)
- 0: 기존과 동일 (어두운 밴드 유지)
- 1: 어두운 밴드를 원본 NdotL 쪽으로 당겨 **완화**
- 적용 위치: `ToonStepEditable()` 내부 (Forward/Deferred 동일)
```
// 가장 어두운 밴드(첫 단계)만 보정
alphaAdj = alpha * (1.0f - toonPbrShadowAlpha * darkMask);
...
- Dark Band Alpha: ToonPBREditable의 **검은 밴드만 완화**
